...
1. 문제 정의</br>
1.1 문제 정의</br>
    멀티샷이 반영되고 기존 미사일과 다른 새로운 무기를 설계하고 구현한다.</br> 보통 비
    행기 슈팅게임에서 적이 발사하는 탄도를 응용하여 계획을 진행했다.</br>
    설계할 신무기는 로켓이다.</br> 이 로켓은 수류탄처럼 폭발이 일어나고, 폭발이 일어난
    곳에서부터 파편이 날라와서 UFO를 제거하는 방식이다.</br> 로켓은 UFO와 충동하지 않
    고, 일정 위치에서 폭발이 일어난다.</br>
    그림에서 보이는 것처럼 파편이 날아가
    는 방향은 위, 아래, 좌, 우 그리고 각 대
    각선 방향으로 총 8가지 방향으로 나아간
    다.</br>
    로켓이 폭발이 일어나면 폭발위치로부
    터 파편 8개가 발사된다.</br>
    파편은 마치 회오리처럼 회색 화살표 방
    향으로 원형을 그리면서 나아간다.</br>
![SpaceWar](./images/rocket1.png)</br>
1.2 개발해야 하는 부분</br>
    * 기존의 미사일 대신 로켓객체를 생성한다.</br>
    * 파편 객체를 생성한다.</br>
    * 발사된 로켓이 랜덤 위치에서 폭발을 일으킨다.</br>
    * 로켓이 BLAST가 진행되면, 파편을 발생시킨다.</br>
    * 파편의 진행방향을 회오리 모양으로 나아가는 식을 구한다.</br>
    * 멀티샷이 지원되기 때문에 파편의 개수를 정한다.</br>
    * 파편과 UFO의 충돌처리를 개발한다.</br>
    * 파편의 진행 속도와 회전속도의 적절한 상태를 구한다.</br>
2. 해결 과정</br></br>
    1) 로켓 객체 생성</br>
        로켓 객체는 UFO와 충돌하지 않고, 램덤으로 정해진 위치에서 폭발을 한다.</br> 기존
        의 미사일과 UFO의 충돌처리에서 getBBox의 intersects를 이용하지 않고, Util의
        `rand(min, max)`를 이용하여 BLAST를 발생하도록 개발했다.</br> min과 max값은 각각 40
        과 400으로 설정하고 로켓의 y좌표와 비교하여 BLAST를 일으켰다.</br></br>
    2) 파편 객체 생성</br>
        파편은 배열 객체이고, 배열의 크기는 `[ 로켓의 개수 * 8 +1 ]` 이다. </br>마지막에 1을
        더해주는 이유는 파편의 회전 각을 정할 때, 식 계산의 편리를 위해서 사용했다.</br>
        파편 객체는 로켓이 BLAST상태가 되면 파편의 함수shot(x, y)을 호출하게 된다.</br> 파
        편의 초기 발사 위치 값은 폭발된 로켓의 x좌표와 y좌표를 넘겨준다.</br>
        그 다음 각 파편의 함수 move(int i)를 불러온다. 여기서 “ i ”는 회전 각의 값이다.</br>
        시작 방향은 총 8개이고, 각 8개의 방향에 대해서 “ degree ”배열을 생성하여, 초기값
        을 설정했다. </br>초기값은 45, 90, 135, 180, 225, 270, 315, 0 순으로 설정했다.</br> 설정할 때
        사용한 식은 `( i%8 )*45`를 사용했다. </br>8로 나눈 이유는 한번에 나오는 파편의 개수가 8
        개이고, degree 배열의 크기는` MAX_ROCKET(로켓의 개수)*8+1` 이기 때문에 8로 나
        누었다.</br>
    3) 파편 발사(원형 회전 방향)</br>
    for (int i = 1; i< MAX_MISSILE; i++) {</br>
    </t>    missile[1].move(degree[i % 8]);</br>
    </t>    degree[i % 8]++;</br>
    </t>    if (degree[i % 8] >= 360)</br>
    </t></t>        [i % 8] = 0;</br>
    }</br>
    
        파편들의 각도를 증가시켜서 회전을 유도한다.</br> 단, degree[ ]의 값이 360이 넘게 되
        면 0과 같은 방향이므로 0으로 초기화 한다.</br> 따라서 값이 계속 커지는 경우를 방지했
        다.</br>
        위의 식은 파편 객체의 move함수안에 내용이다.</br> 파편의 x좌표와 y좌표에 전달받은
        degree값(여기서는 i)을 sin과cos에 넣는다. </br>그 각 값에 dy(회전속도)를 곱한다. </br>따라
        서 degree값이 0에서 360까지 값이 반복하는 값을 전달 받게 되고, 그 값을 각도로
