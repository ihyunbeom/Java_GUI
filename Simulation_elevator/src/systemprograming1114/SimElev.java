...
class Car {
	...
	boolean moveTo(int floor) {
		...
		if (state == CS_OPENING || state == CS_OPENED || state == CS_CLOSING) {
			return false;
		}
	...
	void draw(Graphics g, int tx, int ty) {
		...
		// 카 상태 출력
		g.setColor(Color.BLACK);
		g.drawString("Name = " + name, tx + 10, ty + 20);
		g.drawString("State = " + s_state[state], tx + 10, ty + 40);
		g.drawString("Floor = " + (cur_floor + 1) + "F (" + cur_floor + ")", tx + 10, ty + 60);
		g.drawString("Destination = " + (dest_floor + 1) + "F (" + dest_floor + ")", tx + 10, ty + 80);
		g.drawString("Direction = " + s_dir[direction], tx + 10, ty + 100);
		g.drawString("Height = " + cur_height + " cm", tx + 10, ty + 120);
		g.drawString("Open Timer = " + open_timer, tx + 10, ty + 140);
	}
}
...
class CarButton {
	...
	boolean getButton(int floor) {
		return jbtn_floor[floor].isSelected();
	}
...
class HallButton {
	...
	void setUpButton(int floor, boolean b){
		jbtn_up[floor].setSelected(b); 
	}
	void setDownButton(int floor, boolean b){
		jbtn_down[floor].setSelected(b); 
	}
	
	boolean getUpButton(int floor) {
		return jbtn_up[floor].isSelected();
	}

	boolean getDownButton(int floor) {
		return jbtn_down[floor].isSelected();
	}
	...
	HallButton(SimElev se, int bx, int by) {
		...
		for (int i = 0; i < SimElev.FLOOR_SIZE; i++) {
			int d_top = SimElev.FRAME_DH - i * SimElev.FRAME_DH / SimElev.FLOOR_SIZE - 80;
...
class SimElev extends JPanel {
	...
	////// 과제5//////
	private boolean h4c_up[][]; // 카별 Up 호출 플래그
	private boolean h4c_down[][]; // 카별 Down 호출 플래그

	private Car c4h_up[]; // Up 버튼 소속 카
	private Car c4h_down[]; // Down 버튼 소속 카
	////////////////
	...
	SimElev() {
		...
		////////// 과제5///////////
		// 카별 Up/Down 호출 플래그 초기화
		h4c_up = new boolean[3][];
		h4c_down = new boolean[3][];
		for (int i = 0; i < 3; i++) {
			h4c_up[i] = new boolean[FLOOR_SIZE];
			h4c_down[i] = new boolean[FLOOR_SIZE];
		...
		// 홀 버튼 소속 카 저장소 초기화
		c4h_up = new Car[FLOOR_SIZE];
		c4h_down = new Car[FLOOR_SIZE];
	...
	public void paintComponent(Graphics g) {
		...
		// 카별 Up/Down 호출 플래그 그리기
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < FLOOR_SIZE; j++) {
				g.drawString("Up:" + ((h4c_up[i][j]) ? "0" : " "), bx + 10 + i * 40,
						FRAME_M + 495 + (FLOOR_SIZE - j - 1) * 35);
				g.drawString("Dn:" + ((h4c_down[i][j]) ? "0" : " "), bx + 10 + i * 40,
						FRAME_M + 510 + (FLOOR_SIZE - j - 1) * 35);
			}
	}
	...
	// Up 방향으로 가장 가까운 카 검색
	Car findNearestUp(int floor) {
		int movement[] = { 0, 0, 0 };
		int min = 0;
		System.out.printf("Up Movement =");
		for (int i = 0; i < 3; i++) {
			int state = car[i].getState();
			int cur = car[i].getCurFloor();
			int dir = car[i].getDirection();
			// 각 카에 대한 movement 계산
			if (state == Car.CS_STOP)
				movement[i] = Math.abs(floor - cur);////
			else if (dir == Car.DIR_UP) {
				if (floor > cur)
					movement[i] = floor - cur;////
				else
					// = (FLOOR_SIZE-cur) + FLOOR_SIZE + floor
					movement[i] = (FLOOR_SIZE - cur) + FLOOR_SIZE + floor;////
			} else // dir == Car.DIR_DOWN
				movement[i] = cur + floor + 2;// cur + 1 + floor + 1
			System.out.printf(" %d", movement[i]);
		}

		// movement가 가장 작은 카 검색
		for (int i = 1; i < 3; i++)
			if (movement[min] > movement[i])
				min = i;
	...
	// Down 방향으로 가장 가까운 카 검색
	Car findNearestDown(int floor) {
		int movement[] = { 0, 0, 0 };
		int min = 0;

		System.out.printf("Nearest Down Weight =");
		for (int i = 0; i < 3; i++) {
			int state = car[i].getState();
			int cur = car[i].getCurFloor();
			int dir = car[i].getDirection();
			...
			if (state == Car.CS_STOP)
				...
				if (cur > floor)
					movement[i] = cur - floor;
				else
					// = cur + FLOOR_SIZE + (FLOOR_SIZE-floor)
					movement[i] = cur + FLOOR_SIZE + (FLOOR_SIZE - floor);
			} else // dir == Car.DIR_UP
					// = (FLOOR_SIZE-cur-1) + (FLOOR_SIZE-floor-1)
				movement[i] = (FLOOR_SIZE - cur - 1) + (FLOOR_SIZE - floor - 1);
	...
	int findDownDest(Car c) {
		...
		if (c.getState() != Car.CS_MOVING) {
			if (cb.getButton(cur) == true || getDownFlag(c, cur) == true)
				return cur;
		}
		// 현재 층에서 Down 방향에 눌려있는 가장 가까운 층버튼 또는 Down 홀버튼 검색
		for (int i = cur - 1; i >= 0; i--) {
			if (cb.getButton(i) == true || getDownFlag(c, i) == true)
				return i;
		}
		// Down 방향에 눌려있는 가장 먼 Up 홀버튼 검색
		for (int i = 0; i < cur; i++) {
			if (getUpFlag(c, i) == true)
				return i;
		}
		// 멈취있는 상태에서 현재층의 Up 홀버튼 확인
		if (c.getState() != Car.CS_MOVING) {
			if (getUpFlag(c, cur) == true) {
	...
	void onStopCar(Car c, int floor) {
		...
		if (dir == Car.DIR_UP) {
			...
			setUpFlag(c, floor, false);
		} else { // dir == Car.DIR_DOWN
			hbtn.resetDownButton(floor);
			setDownFlag(c, floor, false);
		}
		...
		// 카의 동적 재배치를 위해 전체 홀버튼 다시 누르기
		for(int i=0;i<FLOOR_SIZE;i++){
			if(hbtn.getUpButton(i) == true){
				hbtn.setUpButton(i,false);
			}
			if(hbtn.getUpButton(i) == false){
				hbtn.setUpButton(i,true);
			}
			if(hbtn.getDownButton(i) == true){
				hbtn.setDownButton(i,false);
				hbtn.setDownButton(i,true);
			}
			if(hbtn.getDownButton(i) == false){
				hbtn.setDownButton(i,true);
			}
		}
	}
	...
	void onUpButton(HallButton hb, int floor, boolean sel) {
		...
		// 카 선택 및 해제
		if (sel == true) {
			c = findNearestUp(floor); // 가장 가까운 카를 찾는다.
			setUpFlag(c, floor, true); // 위에 카의 호출 플래그를 설정한다.
			c4h_up[floor] = c; // 선택된 카를 저장한다.
		} else {
			c = c4h_up[floor]; // 저장된 카를 확인
			c4h_up[floor] = null; // 카 저장소 초기화
			setUpFlag(c, floor, false); // 카별 호출 플래그 해제
		}
	...
	void onDownButton(HallButton hb, int floor, boolean sel) {
		...
		// 카 선택 및 해제
		if (sel == true) {
			c = findNearestDown(floor);// 가장 가까운 카를 찾는다.
			setDownFlag(c, floor, true);// 위에 카의 호출 플래그를 설정한다.
			c4h_down[floor] = c;// 선택된 카를 저장한다.
		} else {
			c = c4h_down[floor];// 저장된 카를 확인
			c4h_down[floor] = null;// 카 저장소 초기화
			setDownFlag(c, floor, false);// 카별 호출 플래그 해제
		}
	...
	// 해당 카에 Up 호출 플래그 확인
	boolean getUpFlag(Car c, int floor) {
		for (int i = 0; i < 3; i++)
			if (c == car[i])
				return h4c_up[i][floor];
		return false; // dummy code
	}

	// 해당 카에 Up 호출 플래그 설정
	void setUpFlag(Car c, int floor, boolean b) {
		for (int i = 0; i < 3; i++)
			if (c == car[i])
				h4c_up[i][floor] = b;
	}

	// 해당 카에 Down 호출 플래그 확인
	boolean getDownFlag(Car c, int floor) {
		for (int i = 0; i < 3; i++)
			if (c == car[i])
				return h4c_down[i][floor];
		return false; // dummy code
	}

	// 해당 카에 Down 호출 플래그 설정
	void setDownFlag(Car c, int floor, boolean b) {
		for (int i = 0; i < 3; i++)
			if (c == car[i])
				h4c_down[i][floor] = b;
	}
